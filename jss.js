$(document).ready(function () {
	...
	$(".sclass").change(function () {
		...
		arr.forEach((element, i) => {
			if (i == sub) {
				element.subhe.forEach((element, i) => {
					$(".fclass").append('<option value="' + i + '">' + element.title + '</option>');
				});
			}
		});
	...
	$(".d").submit(function (e) {
		...
		arr.forEach((element, i) => {
			...
			element.subhe.forEach((ele, i) => {
				if (i == sub) {
					if (!(!!ele.form)) {
						ele.form = [];
					}
					ele.form.push({ 'input': label, 'name': name, 'placeholder': placeholder, 'clas': clas, 'value': value, 'option': option })
				}
