		...
		$(vl.subhe).each(function (keyy, vle) {
			...
			$(vle.form).each(function (keey, vlle) {
				...
				if (vlle.input == 'SELECT') {
					...
					console.log(xa)
					var ya = $('<select class="' + vlle.clas + '"> <option>select</option> ' + vlle.option.split(',').map((opt) => { return `<option>${opt}</option>`; }) + '</select>').appendTo(xa);
					console.log(ya)
					...
					$("main section:nth-child(" + (ke + 1) + ") div:nth-child(" + (keyy + 2) + ")").append('<p><label> ' + vlle.label + ' </label><input type="' + vlle.input + '" name="' + vlle.name + ' " placeholder="' + vlle.placeholder + '"   class="' + vlle.clas + '" value="' + vlle.value + '" option="' + vlle.option + '" disabled="'+ vlle.dis+'">  </input> <button  onclick=myFunction(this)>X</button> </p>');
				}
