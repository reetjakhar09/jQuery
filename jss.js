		...
		$(vl.subhe).each(function (keyy, vle) {
			...
			$(vle.form).each(function (keey, vlle) {
				...
				if (vlle.input == 'SELECT') {
					...
					var ya = $('<select class="' + vlle.clas + '"  disabled="'+ vlle.dis+'" readonly="'+vlle.read+'" required="'+vlle.req+'" > <option>select</option> ' + vlle.option.split(',').map((opt) => { return `<option>${opt}</option>`; }) + ' </select> <button onclick=myFunction(this)>X</button>').appendTo(xa);
					$("main section:nth-child(" + (ke + 1) + ") div:nth-child(" + (keyy + 2) + ")").append(xa);
				}

				else if (vlle.input == 'TEXTAREA'){
					var at = $('<p><label> ' + vlle.label + ' </label>  <textarea  name="' + vlle.name + '"   placeholder="' + vlle.placeholder + '"   class="' + vlle.clas + '" value="' + vlle.value + '" option="' + vlle.option + '" " disabled="'+ vlle.dis+'" readonly="'+vlle.read+'" required="'+vlle.req+'"  >' + vlle.value + '</textarea>  <button onclick=myFunction(this)>X</button> </p>');
	...
	$(".d").submit(function (e) {
		...
		if (abc == 'RADIO') {
			$(res).each(function (ky) {
				$("main section:nth-child(" + subv + ") div:nth-child(" + aaa + ")").append('<p><label> ' + res[ky] + '  </label><input type="' + abc + '" name="' + name + '" placeholder="' + placeholder + '"   class="' + clas + '" value="' + value + '" disabled="'+ dis+'" readonly="'+read+'" required="'+req+'"  </input> <button  onclick=myFunction(this)>X</button> </p>');
			...
			$(res).each(function (k) {
				$("main section:nth-child(" + subv + ") div:nth-child(" + aaa + ")").append('<p><label> ' + res[k] + '  </label><input type="' + abc + '" name="' + name + '" placeholder="' + placeholder + '"   class="' + clas + '" value="' + value + '" option="' + option + '" disabled="'+ dis+'" readonly="'+read+'" required="'+req+'" </input> <button  onclick=myFunction(this)>X</button> </p>');
			...
			var y = $('<select class="' + clas + '" disabled="'+ dis+'" readonly="'+read+'" required="'+req+'"><option>select</option></select> ').appendTo(x);
			$(res).each(function (v) {
			...
			var xt = $('<p><label> ' + label + ' </label>  <textarea  name="' + name + '"   placeholder="' + placeholder + '"   class="' + clas + '" value="' + value + '" option="' + option + '" disabled="'+ dis+'" readonly="'+read+'" required="'+req+'" >' + value + '</textarea> <button onclick=myFunction(this)>X</button> </p>');
			...
			$("main section:nth-child(" + subv + ") div:nth-child(" + aaa + ")").append('<p><label> ' + label + ' </label><input type="' + abc + '" name="' + name + '" placeholder="' + placeholder + '"   class="' + clas + '" value="' + value + '" option="' + option + '" disabled="'+ dis+'" readonly="'+read+'" required="'+req+'" </input> <button  onclick=myFunction(this)>X</button> </p>');
			arr[subv - 1].subhe[efg - 1].form.push({'dis': dis, 'read': read, 'req': req, 'input': abc, 'label': label, 'name': name, 'placeholder': placeholder, 'clas': clas, 'value': value, 'option': option })
			localStorage.setItem('arr', JSON.stringify(arr));
		}
